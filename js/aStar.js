...
class Astar {
  ...
  step(){
    ...
    if(this.closed_set.has(state.key)){
      let msg = "Critical Error. Accessing state that is already in the Closed Set. Should never happen.";
      console.log(msg);
      throw new Error(msg);
    ...
    if(state.equals(this.goal)){ //If state is final, print the path and exit
      state.print_path();
      this.path = state.convert_path(this.initial.x, this.initial.y);
      this.done = true;
      return this.done;
    }
